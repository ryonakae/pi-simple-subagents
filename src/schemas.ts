...
import type { ManagementRequest, NormalizedRequest } from "./types.ts";
...
export const SubagentParams = Type.Object({
  ...
  subagent_type: Type.Optional(Type.String({ description: "Agent name for reference-style run" })),
  prompt: Type.Optional(Type.String({ description: "Delegated task for reference-style run" })),
...
function hasLegacyRunParams(params: RawParams): boolean {
  return "agent" in params || "task" in params || "tasks" in params || "chain" in params;
}

function legacyRunError(): Error {
  return new Error(
    [
      "The legacy subagent run API was removed in v2.",
      'Use subagent({ subagent_type: "reviewer", prompt: "..." }) instead of subagent({ agent: "reviewer", task: "..." }).',
      "For parallel or chain workflows, call subagent multiple times from the parent model.",
    ].join("\n"),
  );
}

export function normalizeSubagentParams(params: RawParams): NormalizedRequest {
  const action = params.action ?? (params.resume && !params.subagent_type ? "resume" : "run");
  ...
  if (hasLegacyRunParams(params)) throw legacyRunError();
  ...
  if (!hasText(params.subagent_type) || !hasText(params.prompt)) {
    throw new Error('Reference-style run requires both subagent_type and prompt. Example: subagent({ subagent_type: "reviewer", prompt: "..." }).');
  }
